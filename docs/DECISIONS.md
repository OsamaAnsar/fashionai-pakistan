...
- The catalogue caches public official-brand Shopify listings so browsing stays reliable; `npm run scrape` refreshes with rate limiting, normalization and a minimum-result safety check.
- Product images and purchase links remain hosted by their source brands; prices and availability are snapshots, not live inventory guarantees.
